...
public class GeneratorTsCreator {

	private static Logger logger = LoggerFactory.getLogger(GeneratorTsCreator.class);
	...
	public static void generate(String protocolPath) throws JsonParseException, JsonMappingException, IOException {
		...
		sb.append("import { net } from \"./LejiSocket\";\n");
		sb.append("import { WsEncoder, WsDecoder } from \"./WebsocketCodec\";\n\n");
		sb.append("export module protocol{\n\n\texport class MyProtocols {\n\n");
		ObjectMapper mapper = new ObjectMapper();
		Map<String, Object> templateAllMap = new HashMap<>();
		for (File jsonFile : filenames) {
			...
			templateAllMap.putAll(templateMap);
		}
		buildMessage(templateAllMap, sb);
		sb.append("\n\t}\n");
		//生成类
		...
		for (File jsonFile : filenames) {
			...
			logger.info("fileName={}", bigFileName);
			Map<String, Object> templateMap = mapper.readValue(jsonFile, Map.class);
			classMap.putAll(templateMap);
		}
		buildClass(classMap, sb);
		sb.append("\n}");
		FileUtils.write(new File(protocolPath+"/ts/MyProtocols.ts"), sb.toString(), Charset.forName("UTF-8"));
	}

	private static void buildClass(Map<String, Object> templateMap, StringBuilder sb) {
	...
	private static void buildMessage(Map<String, Object> templateMap, StringBuilder sb)
			throws IOException {
		for (Map.Entry<String, Object> aEntry : templateMap.entrySet()) {
			String aCalssName = aEntry.getKey();
			Map<String, Object> aObj = (Map<String, Object>) aEntry.getValue();
			...
			if (aObj.get("is_req") != null && (boolean) aObj.get("is_req")) {
				...
				sb.append("\t\t\tlet wsEncoder = WsEncoder.getInstance().init();\n");
				...
				Integer msgCode = (Integer) aObj.get("id");
				if(msgCode != null){
					sb.append("\t\tstatic get_" + msgCode + "(myDecoder:WsDecoder):"+aCalssName+"{\n");
				}else{
					sb.append("\t\tstatic get_" + aCalssName + "(myDecoder:WsDecoder):"+aCalssName+"{\n");
				}
	...
	private static String getTsTypeName(String fieldType) {
		...
		if (fieldType.equals("int")) {
			...
			fieldType = fieldType.substring(2);
			String[] split = StringUtils.split(fieldType, ":");
			String keyTypeName = getTsTypeName(split[0]);
			String valueTypeName = getTsTypeName(split[1]);
			ret = "{[key:"+keyTypeName+"]:"+valueTypeName+"}";
		} else {
			ret = fieldType;
		}
		return ret;
	...
	private static String getReadStr(Map<String, Object> rootMap, String fieldName, String fieldType,
		...
		String ret = "";
		if(isFieldRaw(fieldType)){
			ret = "\tretObj." + fieldName + "="+getReadStrSimple(fieldType)+";\n";
		} else if (fieldType.equals("bytearray")) {
			...
			ret += "\tif(" + fieldNameNew + "_size >0){\n";
			ret += "\t\tfor(var " + indexKey + "=0; "+ indexKey +"<" + fieldNameNew + "_size;"+indexKey+"++){\n";
			ret += "\t\t\tlet "+fieldNameNew+"_key ="+getReadStrSimple(keyType)+";\n";
			ret += "\t\t" + getReadStr(rootMap, fieldName + "[" + fieldNameNew + "_key]", valueType, false, fieldName + "_idx");
			ret += "\t\t}\n";
			...
			ret += "\t\tretObj." + fieldName + " = MyProtocols.get_" + beanObj.getOrDefault("id", fieldType) + "(myDecoder);\n";
...
//			}
	...
	private static String getReadStrSimple(String fieldType){
		String ret = "";
		switch (fieldType) {
			case "int":
				ret = "myDecoder.readInt()";
				break;
			case "short":
				ret = "myDecoder.readShort()";
				break;
			case "byte":
				ret = "myDecoder.readByte()";
				break;
			case "long":
				ret = "myDecoder.readLong()";
				break;
			case "bool":
				ret = "myDecoder.readBool()";
				break;
			case "float":
				ret = "myDecoder.readFloat()";
				break;
			case "string":
				ret = "myDecoder.readString()";
				break;
		}
		return ret;
	}
