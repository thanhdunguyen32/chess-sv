...
@MsgCodeAnn(msgcode = WsMessageActivity.C2SCzlbList.id, accessLimit = 200)
public class CzlbListProcessor extends PlayingRoleMsgProcessor {

    private static Logger logger = LoggerFactory.getLogger(CzlbListProcessor.class);
    ...
    public void processMy(PlayingRole playingRole, MyRequestMessage request) throws Exception {
        ...
        logger.info("get czlb,playerId={}", playerId);
        //
        LibaoBuy libaoBuy = LibaoBuyCache.getInstance().getLibaoBuy(playerId);
        WsMessageActivity.S2CCzlbList respmsg = new WsMessageActivity.S2CCzlbList();
        ...
        for (ZhdCzlbTemplate zhdCzlbTemplate : czlbTemplates) {
            ...
            //购买记录
            int myBuyCount = 0;
            if (libaoBuy != null && libaoBuy.getBuyCount() != null && libaoBuy.getBuyCount().containsKey(PayConstants.PRODUCT_ID_CZLB + idx)) {
                myBuyCount = libaoBuy.getBuyCount().get(PayConstants.PRODUCT_ID_CZLB + idx);
            }
