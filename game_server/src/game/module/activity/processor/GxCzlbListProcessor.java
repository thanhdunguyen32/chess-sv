...
@MsgCodeAnn(msgcode = WsMessageActivity.C2SGxCzlbList.id, accessLimit = 200)
public class GxCzlbListProcessor extends PlayingRoleMsgProcessor {

    private static Logger logger = LoggerFactory.getLogger(GxCzlbListProcessor.class);
    ...
    public void processMy(PlayingRole playingRole, MyRequestMessage request) throws Exception {
        ...
        logger.info("get gxlb,playerId={}", playerId);
        //
        LibaoBuy libaoBuy = LibaoBuyCache.getInstance().getLibaoBuy(playerId);
        WsMessageActivity.S2CGxCzlbList respmsg = new WsMessageActivity.S2CGxCzlbList();
        respmsg.list = new ArrayList<>();
        List<ZhdCzlbTemplate> czlbTemplates = ActivityWeekTemplateCache.getInstance().getGxCzlbTemplates();
        int idx = 0;
        for (ZhdCzlbTemplate zhdCzlbTemplate : czlbTemplates) {
            ...
            //购买记录
            int myBuyCount = 0;
            if (libaoBuy != null && libaoBuy.getBuyCount() != null && libaoBuy.getBuyCount().containsKey(PayConstants.PRODUCT_ID_GXLB + idx)) {
                myBuyCount = libaoBuy.getBuyCount().get(PayConstants.PRODUCT_ID_GXLB + idx);
            }
