...
@MsgCodeAnn(msgcode = WsMessageActivity.C2SGzCzlbList.id, accessLimit = 200)
public class GzCzlbListProcessor extends PlayingRoleMsgProcessor {

    private static Logger logger = LoggerFactory.getLogger(GzCzlbListProcessor.class);
    ...
    public void processMy(PlayingRole playingRole, MyRequestMessage request) throws Exception {
        ...
        logger.info("get gzlb,playerId={}", playerId);
        //
        LibaoBuy libaoBuy = LibaoBuyCache.getInstance().getLibaoBuy(playerId);
        WsMessageActivity.S2CGzCzlbList respmsg = new WsMessageActivity.S2CGzCzlbList();
        respmsg.list = new ArrayList<>();
        List<ZhdCzlbTemplate> czlbTemplates = ActivityWeekTemplateCache.getInstance().getGzCzlbTemplates();
        int idx = 0;
        for (ZhdCzlbTemplate zhdCzlbTemplate : czlbTemplates) {
            ...
            //购买记录
            int myBuyCount = 0;
            if (libaoBuy != null && libaoBuy.getBuyCount() != null && libaoBuy.getBuyCount().containsKey(PayConstants.PRODUCT_ID_GZLB + idx)) {
                myBuyCount = libaoBuy.getBuyCount().get(PayConstants.PRODUCT_ID_GZLB + idx);
            }
