...
public class FebsStartedUpRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        try {
            ...
            ActivityTemplateCache.getInstance().reload();
            ...
            log.error("                        ");
            log.error("Khởi động FEBS không thành công，{}", e.getMessage());
            log.error("Ngoại lệ kết nối Redis, vui lòng kiểm tra cấu hình kết nối Redis và đảm bảo dịch vụ Redis đã được khởi động");
            // 关闭 FEBS
            context.close();
        }
        if (context.isActive()) {
            ...
            log.info("                                                      ");
            log.info("Hệ thống cấp phép FEBS đã được khởi động, địa chỉ: {}", url);
