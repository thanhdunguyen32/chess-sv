layui.extend({
    ...
    self.initView = function (route) {
        ...
        if ($.inArray(route.fileurl, conf.indPage) === -1) {
            ...
            if (view.containerBody == null) {
                ...
                //layout Đã tải tệp, xem tệp đã tải
                loadRenderPage()
            }
        } else {
            //Tải trang đơn
    ...
    //Đánh dấu điều hướng bên trái dựa trên URL hiện được tải
    ...
    self.popup = function (params) {
        ...
        view.loadHtml(conf.views + url, function (res) {
            ...
            if (params.title === undefined) {
                params.title = htmlElem.find('title').text() || 'Thông tin';
    ...
    //Khi nó nhỏ hơn kích thước này, nó sẽ thích ứng với điện thoại di động.
    var mobileWidth = 991;
    ...
    function mobileAdapter() {
        ...
        if (device.weixin || device.android || device.ios) {
            //Nhấp vào khoảng trống để đóng thanh bên
    ...
    $(window).on('hashchange', function (e) {
        //Để chuyển đến một liên kết trên thiết bị di động, trước tiên hãy tắt điều hướng.
        if ($(window).width() < mobileWidth) {
            self.flexible(false)
        }
        self.route = layui.router();
    ...
    function alertParams(msg, params) {
        ...
        params.title = [
            ...
            (msg || 'Vui lòng điền thông tin nhanh chóng'),
    ...
    self.modal.open = function (title, url, params) {
        params = $.extend({
            ...
            shadeClose: false,
            title: [
                (title || 'Vui lòng điền thông tin tiêu đề'),
    ...
    self.download = function (url, params, fileName) {
        ...
        xhr.onload = function () {
            if (this.status === 200) {
                ...
                self.alert.error('Tải xuống không thành công');
            }
        };
        xhr.send();
    };

    // Xác định xem các thuộc tính của loại a có hiện diện trong loại b và có bằng nhau không.
    ...
    function resolveResponse(r, f) {
        if (r.code === 200) {
            ...
            self.modal.info('Đăng nhập không hợp lệ', 'Đăng nhập đã hết hạn, vui lòng đăng nhập lại', function () {
                window.location.href = ctx + 'login';
            });
        } else if (r.code === 403) {
            self.alert.warn('Rất tiếc, bạn chưa có quyền thực hiện thao tác này');
        } else {
            self.alert.error(r.message ? r.message : 'Thao tác không thành công');
            console.error(r);
        }
    }
    ...
    // Phân tích nội dung file BASE64 cho IE, Edge
