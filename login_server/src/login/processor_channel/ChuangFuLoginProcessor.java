...
import ws.WsMessageLogin.C2SChuangFuLogin;

@MsgCodeAnn(msgcode = C2SChuangFuLogin.id, accessLimit = 200)
public class ChuangFuLoginProcessor extends MsgProcessor {

	private static Logger logger = LoggerFactory.getLogger(ChuangFuLoginProcessor.class);
	...
	public void process(GamePlayer session, MyRequestMessage request) throws Exception {
		C2SChuangFuLogin garenaLogin = C2SChuangFuLogin.parse(request);
		...
		logger.info("{}-chuangfu-login,appId={},userId={},token={},appKey={}", C2SChuangFuLogin.id, appId, uin, token, appKey);
		LoginStat.getInstance().saveLoginTime(uin);
		loginAccountManager.yijieSwitchLogin(session, appId, uin, token, appKey, "cf-");
	}
