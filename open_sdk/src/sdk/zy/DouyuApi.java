...
public class DouyuApi {
	...
	private static Logger logger = LoggerFactory.getLogger(DouyuApi.class);

	static class SingletonHolder {
		static DouyuApi instance = new DouyuApi();
	}

	public static DouyuApi getInstance() {
		return SingletonHolder.instance;
	}

	public Map<String, Object> verifylogin(String appkey, int gameid, String uin, String tokenkey, String requestUrl) {
		// 生成sign
		try {
			String md5RawStr = String.format("game_id=%1$d&token=%2$s&uin=%3$s%4$s", gameid, tokenkey, uin, appkey);
			logger.info("md5RawStr={}", md5RawStr);
			String mySighStr = DigestUtils.md5Hex(md5RawStr);
			String requestParams = String.format("game_id=%1$d&token=%2$s&uin=%3$s&sign=%4$s", gameid,
					URLEncoder.encode(tokenkey, "UTF-8"), uin, mySighStr);
	...
	public static void main(String[] args) {
		...
		try {
			Map<String, Object> retMap = DouyuApi.getInstance().getOrderId("18", 1, "diamond001", "diamond001", 600, 5,
