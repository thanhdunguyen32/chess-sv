 ...
 *                  new {@link ProtobufClientDecoder}(MyMessage.getDefaultInstance()));
 * 
 * // Encoder
 ...
 */
@Sharable
public class ProtobufClientDecoder extends MessageToMessageDecoder<ByteBuf> {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) throws Exception {
		...
		int msgCode = CodedInputStream.newInstance(array, offset, length).readRawVarint32();
		int msgCodeLength = CodedOutputStream.computeUInt32SizeNoTag(msgCode);
		int retCode = CodedInputStream.newInstance(array, offset + msgCodeLength, length - msgCodeLength)
				.readRawVarint32();
		int retCodeLength = CodedOutputStream.computeUInt32SizeNoTag(retCode);
		out.add(new ClientInputProtoMessage(msgCode, array, offset + msgCodeLength
				+ retCodeLength, length - msgCodeLength - retCodeLength));
	}
